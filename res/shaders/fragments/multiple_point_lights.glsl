...
//Own function jsut to calculate specular light based off the function formulas above
vec3 SpecCalc(vec3 worldPos, vec3 normal, vec3 viewDir, Light light, float shininess) {
	// Get the direction to the light in world space
	vec3 toLight = light.Position.xyz - worldPos;
	// Get distance between fragment and light
	float dist = length(toLight);
	...
	// Calculate our specular power
	float specPower  = pow(max(dot(normal, halfDir), 0.0), pow(256, 1));
	// Calculate specular color
	vec3 specularOut = specPower * light.ColorAttenuation.rgb;
...
//Function that uses specCalc to return lightAccumulation
vec3 FinalSpecCalc(vec3 worldPos, vec3 normal, vec3 camPos, float shininess) {
    // Will accumulate the contributions of all lights on this fragment
	vec3 lightAccumulation = AmbientColAndNumLights.rgb;
	...
	for(int ix = 0; ix < AmbientColAndNumLights.w && ix < MAX_LIGHTS; ix++) {
		// Additive lighting model
		lightAccumulation += SpecCalc(worldPos, normal, viewDir, Lights[ix], shininess);
	}
